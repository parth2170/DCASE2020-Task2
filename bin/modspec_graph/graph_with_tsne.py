	...
	if mode == 'd':
		...
		for m in machines:
			...
			for mid in mid_dict[m]:
				...
				tsne = TSNE(n_components=2, perplexity=2.0, random_state = 0, n_iter = tsne_iter, metric = 'manhattan')
				X_train_tsne = reshape_(X_train)
				X_train_tsne = tsne.fit_transform(X_train_tsne)

				subgraphs, Graph = build_net(X_train_tsne)
				means, deviations = get_means(X_train, subgraphs)
				y_pred_gr = get_anom_score(X_test, means, deviations)
		...
		results.to_csv('modspec_graph_withTSNE_dev_data_results.csv')
		print(results)
		with open('../ensemble/individual_scores/dev/modspec_graph_withTSNE_dev_data.pickle', 'wb') as file:
		...
		for m in machines:
			...
			for mid in mid_dict[m]:
				...
				submission_file.to_csv('../../task2/Tiwari_IITKGP_task2_2/anomaly_score_{}_id_0{}.csv'.format(m, mid), header = False, index = False)
